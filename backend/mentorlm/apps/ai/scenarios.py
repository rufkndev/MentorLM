"""Реестр сценариев — пресеты «режим внутри режима» (выбор под полем ввода).

Единственный источник правды о том, что сценарий добавляет к режиму: свой
системный промпт и полный набор параметров генерации. Клиент шлёт только id.
"""
...
# Псевдонимы-подсказки: реальные значения перечислены в комментариях, а их
# расшифровки в директивы — в prompts.py.
...
class ScenarioConfig:
    """Пресет сценария: надстройка к промпту режима плюс параметры генерации.

    Параметры — база и границы: настройки пользователя двигают их внутри
    заданного коридора (см. ai.preferences), а не перекрывают.
    """

    id: str
    system_prompt: str
    temperature: float = 0.4
    # Коридор для сдвига настройкой «Креативность»; None → окно ±0.2 вокруг базы.
...
def _s(id: str, system_prompt: str, **params) -> ScenarioConfig:
    """Короткая запись сценария (id и промпт позиционно)."""
    return ScenarioConfig(id=id, system_prompt=system_prompt, **params)


# ── Сценарии по режимам ───────────────────────────────────────────────────────
# Промпт пишем плотно: что сделать, в каком виде, с какими приоритетами.
# Структурные поля дают директивы через prompts.py — в тексте их не дублируем.
...
SCENARIOS: dict[str, dict[str, ScenarioConfig]] = {
    "chat": {
        ...
        # Дефолт режима: универсальный ответ без надстроек.
        "chat": _s(
            "chat",
