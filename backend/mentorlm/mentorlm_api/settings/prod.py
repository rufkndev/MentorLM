...
# ── Почта: Postbox обязателен ─────────────────────────────────────────────────
# На письмах держится восстановление пароля — единственный способ вернуть
# доступ к аккаунту. Прод, поднятый без рабочей почты, выглядит исправным ровно
# до первого «забыл пароль», поэтому ключи требуем на старте, как и всё
# остальное критичное. Как их получить — dev_docs/notes/emailNote.md.
EMAIL_BACKEND = 'django.core.mail.backends.smtp.EmailBackend'
EMAIL_HOST_USER = _require('POSTBOX_KEY_ID')
EMAIL_HOST_PASSWORD = _require('POSTBOX_SECRET')
# Адрес отправителя — подтверждённый в Postbox, иначе сервис отклонит письмо.
DEFAULT_FROM_EMAIL = _require('DEFAULT_FROM_EMAIL')
# Из него собираются ссылки в письмах: на localhost они бы вели в никуда.
PUBLIC_SITE_URL = _require('PUBLIC_SITE_URL').rstrip('/')


# ── Платежи: ключи ЮKassa обязательны ─────────────────────────────────────────
# Без них не создаётся ни один платёж, а деградировать тут некуда: подписки
# просто перестают продаваться, причём молча — ошибка вылезла бы только у
# первого покупателя. Лучше не подняться на старте.
