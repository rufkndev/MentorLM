...
class GenParams:
    ...
    # Пользователь попросил обдумать ответ (переключатель в композере). У
    # Anthropic это отдельный режим thinking, у OpenAI — поднятое усилие
    # рассуждения; и там и там токены размышления входят в output и
    # оплачиваются как обычный вывод.
    thinking: bool = False
