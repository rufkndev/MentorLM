"""Каталог писем — единственное место, где перечислено, что мы вообще шлём.

Одно письмо = одна запись здесь + пара шаблонов `templates/emails/<name>.{html,txt}`.
Тема живёт рядом с шаблоном, а не в вызывающем коде: иначе одно и то же письмо
уходило бы с разными темами из разных мест.

Добавить письмо = добавить запись сюда и два шаблона. Больше нигде ничего
менять не нужно.
"""

from __future__ import annotations

from dataclasses import dataclass


@dataclass(frozen=True)
class Letter:
    """Описание письма: тема и базовое имя пары шаблонов."""

    subject: str
    # Без расширения: sender добавит .txt и .html.
    template: str


# Ключ — то, что передают в mailer.send: короткое имя письма.
LETTERS: dict[str, Letter] = {
    # Подтверждение адреса при регистрации. Без него пользователь не сможет
    # восстановить пароль, а мы не будем уверены, что почта вообще его.
    "verify_email": Letter(
        subject="Подтвердите почту в MentorLM",
        template="emails/verify_email",
    ),
    # Сброс забытого пароля. Единственный способ вернуть доступ к аккаунту.
    "password_reset": Letter(
        subject="Сброс пароля в MentorLM",
        template="emails/password_reset",
    ),
    # Уведомление постфактум: пароль уже изменён. Единственное письмо, которое
    # ничего не предлагает сделать — и в этом его смысл. Смена пароля иначе не
    # оставляет следа, который человек увидит: если доступ к аккаунту забрали
    # (угнали сессию, залезли в почтовый ящик), это письмо — то самое место,
    # где владелец об этом узнает. Поэтому оно уходит после ОБЕИХ смен: и по
    # ссылке из письма, и из настроек.
    "password_changed": Letter(
        subject="Пароль в MentorLM изменён",
        template="emails/password_changed",
    ),

    # ── Подписка и деньги ────────────────────────────────────────────────────
    # Первое из этих писем — не вежливость, а обязанность: п. 7.3 оферты и
    # ст. 16.1 ЗоЗПП требуют предупредить о списании не позднее чем за 24 часа
    # и назвать сумму, дату, тариф и способ отказа. Не отправили — не имеем
    # права списывать, а списали — обязаны вернуть всё (п. 7.6). Поэтому
    # `renewal_notice` завязано на код: billing.payments.renewal_blocked_reason
    # смотрит именно на отметку о его отправке.
    "renewal_notice": Letter(
        subject="Завтра продлим подписку MentorLM",
        template="emails/renewal_notice",
    ),
    "payment_succeeded": Letter(
        subject="Оплата прошла — подписка MentorLM активна",
        template="emails/payment_succeeded",
    ),
    # Зеркало `renewal_notice` для подписок БЕЗ автопродления: за сутки до
    # окончания предупреждаем, что доступ закончится и продлить нужно руками.
    # Юридически не обязательно (мы ничего не списываем), но по смыслу это то
    # же самое обещание «человек не узнаёт о конце подписки постфактум» — и
    # единственное уведомление, которое получит тот, у кого привязка карты не
    # удалась и автопродления поэтому нет.
    "subscription_expiring": Letter(
        subject="Подписка MentorLM заканчивается завтра",
        template="emails/subscription_expiring",
    ),
    # Чек по ФЗ-422 («Мой налог»). Исполнитель — ИП на НПД, ККТ не применяет
    # (п. 2.2 ст. 2 ФЗ-54), поэтому чек не приходит от оператора фискальных
    # данных сам — его формируют вручную, а это письмо и есть та самая
    # «передача чека покупателю», которой требует ч. 1 ст. 14 ФЗ-422.
    "npd_receipt": Letter(
        subject="Чек по оплате MentorLM",
        template="emails/npd_receipt",
    ),
    # Единственное письмо, которое уходит НЕ пользователю, а владельцу сервиса
    # (SUPPORT_EMAIL). Напоминание выписать чеки в «Мой налог»: срок по ФЗ-422
    # ограничен, а следить за ним по логам докера никто не станет.
    "npd_receipts_due": Letter(
        subject="MentorLM: нужно выписать чеки в «Мой налог»",
        template="emails/npd_receipts_due",
    ),
    "renewal_failed": Letter(
        subject="Не удалось продлить подписку MentorLM",
        template="emails/renewal_failed",
    ),
    "subscription_canceled": Letter(
        subject="Автопродление подписки MentorLM отключено",
        template="emails/subscription_canceled",
    ),
    "refund_succeeded": Letter(
        subject="Возврат по подписке MentorLM",
        template="emails/refund_succeeded",
    ),
}
